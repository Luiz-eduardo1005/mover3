...
// Configuração simplificada - usar storage padrão do Supabase para garantir persistência
// O Supabase salva automaticamente a sessão no localStorage com a chave padrão
export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
    persistSession: true,
    autoRefreshToken: true,
    detectSessionInUrl: true,
    // Usar storage padrão do navegador (localStorage) - mais confiável para persistência
